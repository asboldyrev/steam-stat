        ...
        <template v-else-if="game">
            <article class="overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <div class="relative min-h-56 overflow-hidden bg-gray-100 dark:bg-gray-800 sm:min-h-72">
                    <img v-if="heroUrl && !heroError" :src="heroUrl" :alt="game.name" class="absolute inset-0 h-full w-full object-cover" @error="heroError = true" />
                    ...
                    <div class="relative flex min-h-56 flex-col justify-end p-5 text-white sm:min-h-72 sm:p-7">
                        <div class="flex items-end gap-4">
                            <img v-if="clientIconUrl && !iconError" :src="clientIconUrl" :alt="game.name" class="h-16 w-16 rounded-2xl object-contain shadow-xl ring-1 ring-white/20 sm:h-20 sm:w-20" @error="iconError = true" />
                            ...
                            <a :href="game.store_url" target="_blank" rel="noreferrer" class="hidden rounded-xl bg-white/10 px-4 py-2.5 text-sm font-semibold backdrop-blur transition hover:bg-white/20 sm:inline-flex">
            ...
            <article class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                    <div class="space-y-1.5">
                        <span class="text-sm text-gray-600 dark:text-gray-400 mr-2">{{ t('gameDetail.period') }}</span>
