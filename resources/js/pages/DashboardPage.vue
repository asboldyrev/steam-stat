            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-steam shadow-card transition-smooth hover:shadow-hover">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ formatedTotalPlaytime }}</p>
                    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-steam shadow-card">
                ...
                <div class="space-y-4">
                    ...
                    <div v-else v-for="activity in recentActivity.activities" :key="activity.game_id" class="flex items-center gap-4 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 transition-smooth">
                        ...
                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('common.hours', { hours: activity.duration_hours }) }}</span>
                    </div>
                </div>
            </div>
        </div>
...
<script setup>
    ...
    const formatedTotalPlaytime = computed(() => {
        let playtime = stats.value.total_playtime_hours
        let result = []

        if (playtime > 8760) {
            result.push(Math.floor(playtime / 8760) + ' years')
            playtime = playtime % 8760
        }

        if (playtime > 720) {
            result.push(Math.floor(playtime / 720) + ' months')
            playtime = playtime % 720
        }

        if (playtime > 24) {
            result.push(Math.floor(playtime / 24) + ' days')
            playtime = playtime % 24
        }

        result.push(playtime + ' hours')
